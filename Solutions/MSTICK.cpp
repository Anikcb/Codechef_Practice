...
void build(ll N)
{
    for (ll i=0; i<N; i++)STmn[0][i] = A[i],STmx[0][i] = A[i];
    for (ll k = 1; (1 << k)<N; k++)
    {
        for (ll i=0; i+(1<<k)<=N; i++)
        {
            ll x1 = STmn[k-1][i];
            ll y1 = STmn[k-1][i+(1<<k-1)];
            ll x2 = STmx[k-1][i];
            ll y2 = STmx[k-1][i+(1<<k-1)];
            STmx[k][i]=x2>=y2?x2:y2;
            STmn[k][i]=x1<=y1?x1:y1;
        }
    }
}

ll Mxquery(ll i, ll j)
{
    if(i==j)return A[i];
    ll k = log2(j-i);
    ll x = STmx[k][i];
...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        while(q--)
        {
            ...
            double mx=-1e15+1.0;
            ll val1=Mnquery(a,b);
            if(a-1>=0)mx=max(mx,(double)val1+Mxquery(0,a-1));
            if(b+1<n)mx=max(mx,(double)val1+Mxquery(b+1,n-1));
            ll val2=Mxquery(a,b);
            mx=max(mx,val1+((val2-val1)/2.0));
            cout<<fixed<<setprecision(1)<<mx<<endl;
        }
